...
export const Form = ({mode, action}: FormProps) => {
  ...
  return (
    <form action={action} method="POST" className="block w-full" data-netlify="true" onSubmit={onSubmit}>
      ...
      <div className="mb-6">
        ...
        <input type="text" name="company-name" id="company-name" className="block w-full border-none" value={companyName} onChange={e => setCompanyName(e.target.value)} />
      </div>
      <div className="mb-6">
        <label htmlFor="name" className={`block mb-3 ${mode === "primary" && "text-white"}`}>Name</label>
        <input type="text" name="name" id="name" className="block w-full border-none" value={name} onChange={e => setName(e.target.value)} />
      </div>
      <div className="mb-6">
        <label htmlFor="email" className={`block mb-3 ${mode === "primary" && "text-white"}`}>Email Address</label>
        <input type="email" name="email" id="email" className="block w-full border-none" value={email} onChange={e => setEmail(e.target.value)} />
      </div>
      <div className="mb-6">
        <label htmlFor="subject" className={`block mb-3 ${mode === "primary" && "text-white"}`}>Subject</label>
        <select name="subject" id="subject" className="block w-full border-none" value={subject} onChange={e => setSubject(e.target.value)}>
      ...
      <div className="mb-12">
        ...
        <textarea name="message" id="message" rows={5} maxLength={500} className="block w-full border-none" value={message} onChange={e => setMessage(e.target.value)}></textarea>
      </div>
